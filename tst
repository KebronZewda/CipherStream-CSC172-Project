...
public class tester {
    ...
    private static final int[] PERMUTATION_P = {16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25};
    ...
    private static String readBinaryFromFile(String filePath) throws IOException {
        ...
        try (Scanner scanner = new Scanner(new File(filePath))) {
            while (scanner.hasNextLine()) {
                binaryStringBuilder.append(scanner.nextLine().trim());
            }
        }
        return binaryStringBuilder.toString();
    }

    private static void writeBinaryToFile(String filePath, String binaryData) throws IOException {
        try (PrintWriter out = new PrintWriter(filePath)) {
            out.println(binaryData);
        }
    }


    private static String xorIt(String a, String b) {
    ...
    private static String substitutionS(String input) {
        StringBuilder output = new StringBuilder();
        for (int i = 0; i < input.length(); i += 8) {
            int row = Integer.parseInt(input.substring(i, i + 4), 2);
            int col = Integer.parseInt(input.substring(i + 4, i + 8), 2);
            output.append(S[row][col]);
        }
        return output.toString();
    }
    private static String shiftIt(String key) {
        // assuming a circular left shift by 1
        return key.substring(1) + key.charAt(0);
    }

    private static String permuteIt(String input) {
        char[] output = new char[input.length()];
        Arrays.fill(output, '0');
        for (int i = 0; i < PERMUTATION_P.length; i++) {
            int index = PERMUTATION_P[i] - 1;
            output[index] = input.charAt(i);
        }
        return new String(output);
    }
    private static String functionF(String right, String subkey) {
        String afterXOR = xorIt(right, subkey);
        String afterSubstitution = substitutionS(afterXOR);
        return permuteIt(afterSubstitution);
    }

    private static String[] generateSubkeys(String key) {
        String[] subkeys = new String[10];
        for (int i = 0; i < 10; i++) {
            key = shiftIt(key); // Shift the key
            subkeys[i] = key.substring(0, 32); // Take the leftmost 32 bits as the subkey
        }
        return subkeys;
    }

    // Stub for the encryptBlock method
    public static String encryptBlock(String block, String key) {
        ...
        for (int i = 0; i < 10; i++) {
            String newRight = xorIt(left, functionF(right, subkeys[i]));
            left = right;
            right = newRight;
        }
        return left + right; // Concatenate the two halves
    }
    ...
    public static String decryptBlock(String block, String key) {
        ...
        for (int i = 9; i >= 0; i--) {
            String newLeft = xorIt(right, functionF(left, subkeys[i]));
            right = left;
            left = newLeft;
        }
        return left + right; // Concatenate the two halves
    ...
    public static void main(String[] args) {
        String inputFilePath = "data.txt"; // Update with the actual path
        String outputFilePath = "string.txt"; // Update with the actual path

        try {
            String binaryInput = readBinaryFromFile(inputFilePath);
            System.out.println("Binary input read from file: " + binaryInput);

            // Assuming key is defined and encryption/decryption logic is handled...
            String result = binaryInput; // Placeholder, replace with actual encryption/decryption result

            writeBinaryToFile(outputFilePath, result);
            System.out.println("Binary output written to file.");
        } catch (IOException e) {
